...
export const ResetPasswordView: FC = () => {
  const resetPasswordReq = useConnectedAction(authActions.resetPasswordReq);

  const isFetching = useSelector((state: RootState) => getReqPending(state, authActions.resetPasswordReq.type));

  const [isSuccess, setSuccess] = useState<boolean>(false);
  const [error, setError] = useState<string>('');

  const submitHandler = (values: IResetPasswordFormValues): void => {
    setError('');
    resetPasswordReq(
      { values },
      {
  ...
  return (
    <>
      {isSuccess ? (
        <FormStatusBlock text="Email sended successfully" backLink={RoutePathsEnum.AUTH_SIGN_IN} />
      ) : (
        <ResetPasswordForm submitFetching={isFetching} onSubmit={submitHandler} error={error} />
      )}
    </>
  );
};
